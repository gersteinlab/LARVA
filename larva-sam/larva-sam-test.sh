...
do
	echo "<== TEST CASE $i ==>"
	perl larva-sam.pl $testdir/t$i/vfiles.txt $testdir/t$i/afiles.txt 8 8 g $testdir/t$i/test_db.db
done

for i in {3..5} # Normal range: 3..5
do
# 	echo "<== TEST CASE $i (serial) ==>"
# 	perl larva-sam.pl $testdir/t$i/vfiles.txt $testdir/t$i/afiles.txt 8 1 g $testdir/t$i/test_db.db
